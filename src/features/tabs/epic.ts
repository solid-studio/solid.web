...
export const openTabEpic = (action$: ActionsObservable<TabAction>, state$: StateObservable<ApplicationState>) => action$.pipe(
...
export const setActiveTabEpic = (action$: ActionsObservable<TabAction>, state$: StateObservable<ApplicationState>) => action$.pipe(
    ofType(ActionType.OPEN_OR_SET_ACTIVE_TAB),
    filter(({ payload }) => {
        return state$.value.tabsManagerState.tabs.findIndex((item: any) => {
            return item.id == payload.id
