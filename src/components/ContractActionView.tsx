            ...
            <Container>
                <StyledInputGroup>
                    {inputs && inputs.map((item, index) => {
                        return <StyledInput value={fields[index]} onChange={(e) => {
                            console.log("Index", index)
                            setField({ ...fields, [index]: e.target.value })
                        }
                        } key={index} size="small" style={{ width: '35%' }} type="text" placeholder={`${item.type} ${item.name}`} />
                    })}
                </StyledInputGroup>
            </Container>
            <Container>
                <StyledButton onClick={sendAction} type="primary">{isReadOnlyFunction(abi) ? "Call" : "Send"}</StyledButton>
            </Container>
        </Wrapper>
    )
}
